...
<html lang="en">
<head> 
  <title>Senarai Yang Berhormat</title>
  <meta charset="utf-8">
...
<li class="nav-item">
  ...
  <li class="nav-item">
    ...
    <div class="col-sm-9">
      <h2><small>Senarai Yang Berhormat</small></h2>
      <hr>
      @if (session('status'))
            ...
            @endif
              ...
              <div class="form-group">
                <input type="search" name="search" id="" class="form-control" placeholder="Sila isi nama" />
          
            <button class="btn btn-primary"> &nbsp Carian &nbsp </button>
            <a href="{{url('admin/ybAdd')}}" class="btn btn-success">Tambah YB</a></td>
            </div>
            </form> 
            ...
            <table class="table table-striped">
                <tr>
                    <th>BIL.</th>
                    <th>Nama Yang Berhormat</th> 
                    <th>Nama Parlimen</th>
                    <th>Tindakan</th>
                </tr>
                @if(!empty($ybs) && $ybs->count())
             @foreach($ybs as $yb)
                <tr> 
                <td scope="row">{{ $ybs->firstItem() + $loop->index }}</td>
                    <td>{{$yb['nama']}}</td>
                    <td>{{$yb['nama_parlimen']}}</td>        
                    <td>
                    <a href="{{url('admin/ybEdit',$yb->id)}}" class="btn btn-success">Kemaskini YB</a></td>
  </tr>
  @endforeach   
  @else
                <tr>
