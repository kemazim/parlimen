...
<html lang="en"> 
<head>
  <title>Maklumat Pengguna Aktif</title>
  <meta charset="utf-8">
...
<li class="nav-item">
 ...
 <div class="col-sm-9">
      <h2><small>Senarai Pengguna Aktif</small></h2>
      <hr>
            @if (session('status'))
            ...
            <table class="table table-striped">
             ...
             @foreach($users as $user)
                <tr>
                    ...
                    <td>{{$user['perananString']}}</td>
                    <td>{{$user['statusString']}}</td>
                    <td>{{$user->created_at->format('d-m-Y')}}</td>
                    <td>
                      <a href="{{url('admin/userListEdit/'.$user->id)}}" class="btn btn-success"> Kemaskini</a>
                      <a href="{{url('admin/userListDelete/'.$user->id)}}" class="btn btn-danger" onclick="return confirm('Hapus pengguna?')"> &nbsp&nbsp Hapus &nbsp&nbsp</a>
                    </td>
                </tr>
                @endforeach
            @else
